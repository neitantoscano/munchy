...
export async function actualizarRachaPorGeneracion(supabase, userId) {
  try {
    ...
    // Sacamos la fecha de la última generación (solo la parte AAAA-MM-DD)
    // OJO: usamos null si la racha está en 0, porque eso significa que
    // todavía no ha generado ninguna receta (aunque ya tenga ultima_visita
    // por el registro inicial del usuario).
    const yaHabiaGenerado = nuevaRacha > 0 && usuario.ultima_visita

    if (!yaHabiaGenerado) {
      // Primera receta que genera en su vida → racha arranca en 1
      nuevaRacha = 1
      nuevoRecetasHoy = 1
    } else {
      // Comparar con la última generación
