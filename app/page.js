...
// ——— DATOS DE PRUEBA DE LA DESPENSA (MUNCHY BETA) ———
const DESPENSA_MOCK = [
...
export default function PantallaDashboard() {
  ...
  // Funciones para sumar y restar gramos/piezas
  ...
  // El botón mágico que consume las recetas gratis
    ...
    <div className="min-h-screen bg-[#F4F3EF] text-[#4A3B32] font-sans antialiased pb-10">
      ...
      <header className="max-w-md mx-auto bg-[#2E3A23] text-[#F4F3EF] p-4 rounded-b-2xl shadow-md flex justify-between items-center">
        ...
        <div className="flex items-center space-x-3">
          <span className="text-xs bg-[#4A3B32] px-2 py-1 rounded-full text-[#E9967A] font-semibold">
            Gratis: {recetasGratis} restantes
          </span>
      ...
      <main className="max-w-md mx-auto px-4 mt-6 space-y-6">
        ...
        {/* 🦉 VENTANA FLOTANTE DE LA MASCOTA MUNCHIE (ESTILO DUOLINGO) */}
        {mostrarMascota && (
          <div className="bg-[#2E3A23] text-[#F4F3EF] p-5 rounded-2xl border-2 border-[#4A3B32] shadow-lg relative transition-all animate-bounce">
            <div className="flex items-start space-x-4">
              <div className="w-16 h-16 bg-[#F4F3EF] rounded-xl flex items-center justify-center text-3xl shadow-inner border border-[#4A3B32]">
                🦉
              </div>
              <div className="flex-1 space-y-1">
                <h4 className="font-serif font-bold text-base text-[#E9967A]">¡Qué onda, soy Munchie!</h4>
                <p className="text-xs text-gray-200 leading-relaxed">
                  Hoy andamos modo fit pero rico. Revisa tu alacena de abajo, agrega lo que tengas y te armo un postre premium en 2 segundos. ¡No vayas a apagar el fuego de tu racha!
                </p>
              </div>
            </div>
            <button 
              onClick={() => setMostrarMascota(false)}
              className="absolute top-2 right-2 text-gray-400 hover:text-[#F4F3EF] text-sm font-bold p-1"
            >
              ✕
            </button>
          </div>
        ...
        {/* 🗄️ SECCIÓN DE LA DESPENSA / REFRI VIRTUAL */}
        <section className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm space-y-4">
          <div className="flex justify-between items-center border-b border-gray-100 pb-2">
            ...
            <span className="text-xs text-gray-400">Toca para ajustar gramos</span>
          </div>
          ...
          <div className="space-y-3">
            {despensa.map((item) => (
              <div key={item.id} className="flex justify-between items-center p-3 bg-[#F4F3EF] rounded-xl border border-gray-100">
                ...
                <div className="flex items-center space-x-3">
                  <button 
                    onClick={() => modificarCantidad(item.id, item.unidad === "pzas" ? -1 : -50)}
                    className="w-7 h-7 bg-white rounded-full border border-gray-300 flex items-center justify-center font-bold text-gray-600 active:bg-gray-100 text-xs"
                  >
                    -
                  </button>
                  <span className="text-sm font-bold min-w-[50px] text-center text-[#2E3A23]">
                    {item.cantidad} {item.unidad}
                  </span>
                  <button 
                    onClick={() => modificarCantidad(item.id, item.unidad === "pzas" ? 1 : 50)}
                    className="w-7 h-7 bg-[#2E3A23] text-white rounded-full flex items-center justify-center font-bold active:bg-[#4A3B32] text-xs"
                  >
                    +
                  </button>
                </div>
          ...
          {/* BOTÓN MAESTRO ACCIÓN PRINCIPAL */}
          <button 
            onClick={manejarGenerarReceta}
            className="w-full bg-[#2E3A23] text-[#F4F3EF] py-4 rounded-xl font-bold tracking-wide text-sm shadow-md active:scale-[0.98] transition-all border-b-4 border-[#1c2415]"
          >
            ⚡ GENERAR ANTOJO SALUDABLE
          </button>
        </section>

        {/* ✨ PANTALLA DINÁMICA DE LA RECETA GENERADA */}
        {recetaGenerada && (
          <section className="bg-white p-5 rounded-2xl border-2 border-[#2E3A23] shadow-md space-y-4 transition-all">
            ...
            <p className="text-xs bg-[#2E3A23] text-[#F4F3EF] p-2 rounded-lg text-center font-mono">
              {recetaGenerada.macros}
            </p>

            <div className="space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400">¿Cómo se prepara?</h4>
            ...
            {/* SECCIÓN INTELIGENTE "TE FALTA" / AFILIACIÓN */}
            <div className="p-3 bg-orange-50 rounded-xl border border-orange-200 space-y-2">
              <p className="text-xs font-semibold text-orange-800 flex items-center space-x-1">
                <span>⚠️</span> <span>Te hace falta Proteína y Plátanos para que sepa premium.</span>
              </p>
              <button className="w-full bg-[#E9967A] text-[#4A3B32] py-2 rounded-lg font-bold text-xs shadow-sm active:bg-[#d48569] border border-[#4A3B32]">
                🛒 Surtir despensa barato en Walmart
              </button>
            </div>
          </section>
        )}

        {/* 💳 PANTALLA DE BLOQUEO PREMIUM (PAYWALL DE MERCADO PAGO OBLIGATORIO) */}
        {mostrarPaywall && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-[#F4F3EF] max-w-sm w-full p-6 rounded-2xl border-2 border-[#4A3B32] shadow-2xl text-center space-y-5 relative">
              
              <div className="w-16 h-16 bg-[#2E3A23] rounded-full flex items-center justify-center mx-auto text-2xl shadow-md">
                🔒
              </div>

              <div className="space-y-2">
                <h3 className="font-serif font-bold text-2xl text-[#2E3A23]">Modo Antojo Premium</h3>
                <p className="text-xs text-gray-600 leading-relaxed px-2">
                  Se te acabaron tus 3 recetas gratis del día. No dejes morir tu racha de fueguitos. Desbloquea recetas de helados de proteína y postres ilimitados.
                </p>
              </div>

              <div className="p-4 bg-white rounded-xl border border-gray-200 shadow-inner">
                <span className="text-xs uppercase tracking-wider text-gray-400 block font-bold">Suscripción Mensual</span>
